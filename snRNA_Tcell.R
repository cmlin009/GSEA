...
GSEA_GO5_PN <- gseGO(GSEA_input5_PN, ont = "ALL", OrgDb = GO_database, pvalueCutoff = 1) #GSEA富集分析
...
GSEA_KEGG5_HL <- gseKEGG(GSEA_input5_HL, organism = KEGG_database, pvalueCutoff = 1) #GSEA富集分析
write.csv(GSEA_KEGG5_HL, 'CHRNA5_snRNA_GSEA_KEGG_Tcell_high_low.csv')

GSEA_GO5_HL <- gseGO(GSEA_input5_HL, ont = "ALL", OrgDb = GO_database, pvalueCutoff = 1) #GSEA富集分析
...
gene7_PN <- rename (gene7_PN, "SYMBOL" = 'Gene') #不知為何變成舊的名稱在前面了
...
GSEA_KEGG7_PN <- gseKEGG(GSEA_input7_PN, organism = KEGG_database, pvalueCutoff = 1) #GSEA富集分析
write.csv(GSEA_KEGG7_PN, 'CHRNA7_snRNA_GSEA_KEGG_Tcell_positive_negative.csv')

GSEA_GO7_PN <- gseGO(GSEA_input7_PN, ont = "ALL", OrgDb = GO_database, pvalueCutoff = 1) #GSEA富集分析
...
GSEA_KEGG7_HL <- gseKEGG(GSEA_input7_HL, organism = KEGG_database, pvalueCutoff = 1) #GSEA富集分析
write.csv(GSEA_KEGG7_HL, 'CHRNA7_snRNA_GSEA_KEGG_Tcell_high_low.csv')

GSEA_GO7_HL <- gseGO(GSEA_input7_HL, ont = "ALL", OrgDb = GO_database, pvalueCutoff = 1) #GSEA富集分析
